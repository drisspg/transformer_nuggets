...
class Hyperparameters(transformer_nuggets.llama.train.Hyperparameters):
    ...
    scale: float = 1.0
    proj_type: str = "std"   # TODO, list out what else this could be
...
def main(
    ...
    if hyper_params.use_galore:
        ...
        param_groups = [{"params": regular_params},
                        {"params": galore_params, "rank": hyper_params.rank, "update_proj_gap": hyper_params.update_proj_gap, "scale": hyper_params.scale, "proj_type": hyper_params.proj_type}]
    ...
    # TODO: enable layerwise optim in bwd :D
    ...
    optimizer = optim_cls(
        ...
        # foreach=hyper_params.foreach_optimizer,   TODO: galore optim foreachified?
    )
...
def train(
    ...
    val_loss_file = (
        ...
        / f"galore_validation_loss_{dtype_str}_overfit_{training_config.overfit}_compile_{training_config.compile}_{rank}.csv"
    )
    train_loss_file = (
        training_config.log_dir
        / f"galore_train_loss_{dtype_str}_overfit_{training_config.overfit}_compile_{training_config.compile}_{rank}.csv"
    )
    if rank == 0:
