...
class QloraMLP(nn.Module):
    ...
    def __init__(self, weight1: torch.Tensor, weight2: torch.Tensor, weight3: torch.Tensor, QloraConfig: QloraConfig = None) -> None:
        ...
        self.qlora_w1 = qlora.QloraLinear(weight1.shape[1], weight1.shape[0], weight1, lora_r, lora_alpha, lora_dropout)
        self.qlora_w2 = qlora.QloraLinear(weight2.shape[1], weight2.shape[0], weight2, lora_r, lora_alpha, lora_dropout)
        self.qlora_w3 = qlora.QloraLinear(weight3.shape[1], weight3.shape[0], weight3, lora_r, lora_alpha, lora_dropout)
...
def main(
    ...
    with training_config.device:
        ...
        swap_for_qlora_jank(model, qlora_config, torch.bfloat16)
