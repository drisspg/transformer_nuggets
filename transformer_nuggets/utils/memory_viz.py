...
def _frames_fmt(frames, full_filename=False, reverse=False):
    ...
    return [
        _frame_fmt(f, full_filename) for f in frames if _frame_filter(f["name"], f["filename"])
    ]
    ...
    for seg in data["segments"]:
        ...
        for b in seg["blocks"]:
            if b["state"] == "active_allocated" and addr not in addr_to_alloc:
                ...
                extra = (
                    () if frames else ("<block was allocated before _record_history was enabled>",)
                )
...
if __name__ == "__main__":
    ...
    def _output(p):
        p.add_argument(
            "-o", "--output", default="output.svg", help="flamegraph svg (default: output.svg)"
        )
