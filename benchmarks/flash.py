...
@triton.jit
def rel_attention_triton(cur, m, n, h):
    bias = n - m
    cur = cur + bias
    return cur
...
def alibi_attention_triton(cur, m, n, head_num, num_heads):
    alibi_scale = tl.math.exp2(-(head_num * 8.0 / num_heads))
    bias = n - m
...
def _fwd_kernel(
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if BIAS_CHOICE == BiasMode.rel_pos:
            qk = rel_attention_triton(qk, offs_m[:, None], (start_n + offs_n[None, :]), off_hz)
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, causal, sm_scale, bias_choice: BiasMode):
        BLOCK = 128
        # shape constraints
...
@pytest.mark.parametrize('bias_choice', [BiasMode.rel_pos, BiasMode.alibi, BiasMode.none])
def test_op(Z, H, N_CTX, D_HEAD, causal, bias_choice, dtype=torch.float16):
    ...
    # q = torch.ones((Z, H, N_CTX, D_HEAD), dtype=dtype, device="cuda").requires_grad_()
    # k = torch.ones((Z, H, N_CTX, D_HEAD), dtype=dtype, device="cuda").requires_grad_()
    ...
    tri_out = attention(q, k, v, causal, sm_scale, bias_choice).half()
...
def bench_flash_attention(BATCH, H, N_CTX, D_HEAD, causal, mode, provider, dtype=torch.float16, device="cuda"):
    ...
    if provider == 'pytorch':
        ...
        k = torch.randn((BATCH, H, N_CTX, D_HEAD), dtype=dtype, device="cuda", requires_grad=True)
        v = torch.randn((BATCH, H, N_CTX, D_HEAD), dtype=dtype, device="cuda", requires_grad=True)
        fn = lambda: torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=causal, scale=1.3)
        if mode == 'bwd':
            o = fn()
            do = torch.randn_like(o)
            fn = lambda: o.backward(do, retain_graph=True)
